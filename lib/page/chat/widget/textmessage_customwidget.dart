...
class TextMessageCustomWidget extends StatelessWidget {
  ...
  /// if [highLightWords] is provided, and not added [StringContainsElementType.Str] in [types],
  /// then [StringContainsElementType.Str] will be added to [types] automatically.
  ...
  List<StringContainsElementType> _getAllFilterTypes() {
    ...
    if (types.isEmpty && highLightWords.isEmpty) {
      ...
      return [StringContainsElementType.Str];
      ...
      filterTypes.add(StringContainsElementType.Str);
      return filterTypes.toSet().toList();
    } else if (types.isNotEmpty && highLightWords.isEmpty) {
      filterTypes.addAll(types);
      filterTypes.remove(StringContainsElementType.none);
      filterTypes.remove(StringContainsElementType.Str);
      return filterTypes.toSet().toList();
    }
    return filterTypes;
  }
    ...
    source.splitMapJoin(
      ...
      onMatch: (match) {
        ...
        textSpans.add(
          TextSpan(
            text: matchString,
            style: stringContainsElement.type == StringContainsElementType.Str ? styleForWords : styleForLinks,
            locale: locale,
            semanticsLabel: semanticsLabel,
